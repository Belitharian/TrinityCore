...
enum MageSpells
{
	...
	SPELL_MAGE_ARCANE_SPLINTER                   = 443763,
...
class spell_mage_clearcasting : public AuraScript
{
	...
	{
		eventInfo.GetActor()->CastSpell(eventInfo.GetActor(), SPELL_MAGE_CLEARCASTING_BUFF, CastSpellExtraArgsInit
			{
...
inline void ScheduleSplinterSalvo(Unit* caster, Unit* target, uint32 splinter, uint32 count)
{
	...
	{
		caster->m_Events.AddEventAtOffset([caster, splinter, targetGuid]
		{
			Unit* victim = ObjectAccessor::GetUnit(*caster, targetGuid);
			if (victim && victim->IsAlive())
				caster->CastSpell(victim, splinter, SPLINTER_FLAGS);
		}, offset);
...
// Echelonne `orbAmount` casts de `orbSpell` sur le caster lui-meme,
...
class spell_mage_arcane_missiles : public AuraScript
{
	...
	void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
	{
		...
		if (caster && target)
			caster->CastSpell(target, SPELL_MAGE_ARCANE_MISSILES_DAMAGE, CastSpellExtraArgsInit
				{
					.TriggerFlags = TRIGGERED_IGNORE_CAST_IN_PROGRESS | TRIGGERED_DONT_REPORT_CAST_ERROR
				});
	}
	...
	void Register() override
	{
		...
		OnEffectRemove += AuraEffectRemoveFn(spell_mage_arcane_missiles::ConsumeClearCasting, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY, AuraEffectHandleModes::AURA_EFFECT_HANDLE_REAL);
	}
};
