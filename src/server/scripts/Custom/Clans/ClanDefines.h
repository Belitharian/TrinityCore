...
namespace Clan
{
    ...
    // ---------------------------------------------------------------------
    // Etat discret = (besoin le plus urgent) x (jour/nuit) x 4 bits de contexte :
    // hasRawFood, hasWood, hasStone, litFireNearby. Ces bits permettent d'apprendre
    // la chaine chasser -> (rallumer) -> cuire -> manger.
    constexpr uint8  ACTION_COUNT     = uint8(ActionType::Count);   // 11
    constexpr uint8  NEED_STATE_COUNT = uint8(NeedType::Count);     // 5 (None..Repro)
    constexpr uint8  TIME_STATE_COUNT = 2;                          // jour / nuit
    constexpr uint8  FLAG_STATE_COUNT = 2 * 2 * 2 * 2;             // 16 (4 booleens de contexte)
    constexpr uint16 STATE_COUNT      = uint16(NEED_STATE_COUNT * TIME_STATE_COUNT * FLAG_STATE_COUNT); // 5*2*16 = 160
    ...
    constexpr float Q_EPSILON_START = 0.90f; // exploration initiale
    ...
    // ---------------------------------------------------------------------
    constexpr float  RESOURCE_SEARCH_RANGE = 80.0f;  // rayon de recherche des ressources
    ...
    constexpr uint32 COOK_DURATION_MS      = 5000;   // duree de cuisson sur un feu
    ...
    constexpr uint32 WOOD_RESPAWN_MS        = 60000;  // respawn d'un noeud de bois epuise
    constexpr uint32 ROCK_RESPAWN_MS        = 90000;  // respawn d'un noeud de roche epuise
    ...
    constexpr uint32 SAVE_INTERVAL_MS = 5 * 60 * 1000;
